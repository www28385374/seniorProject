...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href="https://www.hccc.gov.tw/"+dict_all_content['all_href'][i]
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":6}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath('//*[@class="news-card-list"]//li[@class="item"]'))
        all_title=html.xpath('//*[@class="news-card-list"]//li[@class="item"]//a//@title')
        all_href=html.xpath('//*[@class="news-card-list"]//li[@class="item"]//a//@href')
        all_date=html.xpath('//*[@class="news-card-list"]//*[@class="date"]//text()')
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="manual-script-blk"]//text()'))
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://www.hccc.gov.tw/zh-tw/News/List?cls=294&page=1")
        num_page=fn_get_num_page.get_num_page(html_home,'//*[@class="nums"]//text()')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://www.hccc.gov.tw/zh-tw/News/List?cls=294&page={i+1}")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("hualienNews",list_total)
