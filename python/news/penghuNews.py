...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> None:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href=f"https://www.phhcc.gov.tw/{dict_all_content['all_href'][i]}"
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":14}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath('//*[@class="con"]/div'))
        all_title=html.xpath('//*[@class="con"]/div//a//@title')
        all_href=html.xpath('//*[@class="con"]/div//a//@href')
        all_date=html.xpath('//*[@class="con"]//*[@class="date"]//text()')
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="con"]//text()'))
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://www.phhcc.gov.tw/home.jsp?id=59")
        num_page=fn_get_num_page.get_num_page(html_home,'//*[@class="red"]//text()')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://www.phhcc.gov.tw/home.jsp?id=59&pagenum={i+1}&pagesize=100")
            fn_merge_page_content.merge_page_content(html_page)
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("penghuNews",list_total)
