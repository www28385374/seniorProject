import requests,math,re,json,time,random
...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href="https://culture.tycg.gov.tw/"+dict_all_content['all_href'][i]
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":16}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath('//*[@class="css_tr list_list"]'))
        all_title=html.xpath('//*[@class="css_td list_title"]//@title')
        all_href=html.xpath('//*[@class="css_td list_title"]//a//@href')
        all_date=html.xpath('//*[@class="css_td list_date"]//text()')
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="content_detail_row"][last()]/div[2]//text()'))
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://culture.tycg.gov.tw/home.jsp?id=91&parentpath=0,16")
        num_page=fn_get_num_page.get_num_page(html_home,'//*[@for="pages"]/following-sibling::span//text()')
        fn_merge_page_content=GetAllContent()
        # html_page=fn_merge_page_content.get_html(f"https://culture.tycg.gov.tw/home.jsp?id=91&parentpath=0%2C16&mcustomize=&page=1")
        # result=fn_merge_page_content.merge_page_content(html_page)
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://culture.tycg.gov.tw/home.jsp?id=91&parentpath=0%2C16&mcustomize=&page={str(i+1)}")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("taoyuanNews",list_total)
