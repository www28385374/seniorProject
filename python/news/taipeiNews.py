...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href=f"https://culture.gov.taipei/{dict_all_content['all_href'][i]}"
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":18}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath('//*[@id="CCMS_Content"]//tr/td[1]'))
        all_title=html.xpath('//*[@id="CCMS_Content"]//tr/td[2]/span//text()')
        all_href=html.xpath('//*[@id="CCMS_Content"]//tr/td[2]/span/a/@href')
        all_date=html.xpath("//*[@id='CCMS_Content']//tr/td[3]/span/text()")
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://culture.gov.taipei/News.aspx?n=63F36AA6E434AD08&sms=78D644F2755ACCAA&page=1&PageSize=20")
        num_page=fn_get_num_page.get_num_page(html_home,'//span[@class="count"]/text()')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://culture.gov.taipei/News.aspx?n=63F36AA6E434AD08&sms=78D644F2755ACCAA&page={i+1}&PageSize=200")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("taipeiNews",list_total)
