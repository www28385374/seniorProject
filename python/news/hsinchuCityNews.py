...
class GetPageByXpath(BaseFunc):
    ...
    def get_num_page(self, html:object, cmd:str) -> int :
        ...
        num_page=math.ceil(int(num_page)/100)
        return num_page
...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href="https://culture.hccg.gov.tw/ch/"+dict_all_content['all_href'][i]
            content=self.get_article_content(href)
            sig_dict={"title":title,"href":href,"upload_date":upload_date,"content":content,"city_id":4}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        all_title = html.xpath('//*[@class="css_td list_title"][1]/a/text()')
        all_href = html.xpath('//*[@class="css_td list_title"][1]/a/@href')
        all_date = html.xpath('//*[@class="css_td list_date"][1]/text()')
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="content_detail_column02"]//text()'))
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://culture.hccg.gov.tw/ch/home.jsp?id=8&parentpath=0,2&mcustomize=")
        num_page=fn_get_num_page.get_num_page(html_home,'//*[@id="pageno_botton"]/div/ul[2]/li/div/span[1]/text()')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://culture.hccg.gov.tw/ch/home.jsp?id=8&parentpath=0,2&mcustomize=&pagesize=100&serno=null&page={i+1}")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("hsinchuCityNews",list_total)
