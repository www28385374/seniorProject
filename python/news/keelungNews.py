...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            ...
            href="https://www.klccab.gov.tw/"+dict_all_content['all_href'][i]
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":8}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath("//*[@class='news_list']//li"))
        all_title=html.xpath("//*[@class='news_list']//li//a//text()")
        all_href=html.xpath("//*[@class='news_list']//li//a//@href")
        all_date=html.xpath("//*[@class='news_list']//li//a//following-sibling::span//text()")
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="word_info"]//text()'))
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://www.klccab.gov.tw/NewsAndAnnouncement?type=6")
        num_page=fn_get_num_page.get_num_page(html_home,'//*[@class="PagedList-skipToLast"]//@href')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://www.klccab.gov.tw/NewsAndAnnouncement?page={i+1}&type=6")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("keelungNews",list_total)
