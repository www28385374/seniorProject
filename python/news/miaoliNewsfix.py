...
class GetPageByXpath(BaseFunc):
    ...
    def get_num_page(self, html:object, cmd:str) -> int :
        ...
        num_page=int(re.sub(r".*,,,,,,,,","",num_page))
        return num_page
...
class GetAllContent(BaseFunc):
    ...
    def merge_page_content(self, html:object) -> bool:
        ...
        for i in range(dict_all_content['num_data']):
            upload_date=dict_all_content['all_date'][i+1]
            if self.compare_date(upload_date):
                return False
            title=dict_all_content['all_title'][i]
            href="https://www.mlc.gov.tw/latestevent/"+dict_all_content['all_href'][i]
            content=self.get_article_content(href)
            sig_dict={"title":title, "href":href, "upload_date":upload_date, "content":content, "city_id":11}
    ...
    def get_page_all_content(self, html:object) -> dict:
        ...
        """
        num_data=len(html.xpath('//*[@class="list_word w65 text_le"]'))
        all_title=html.xpath('//*[@class="list_word w65 text_le"]//a//text()')
        all_href=html.xpath('//*[@class="list_word w65 text_le"]//a//@href')
        all_date=html.xpath('//*[@class="w20 hidden-xs"]//text()')
    ...
    def get_article_content(self, href:str) -> str:
        ...
        try:
            content="".join(html.xpath('//*[@class="content_txt clearfix"]//text()'))
            content=self.filter_space(content).split("：新聞公告")[1].strip()
        except:
            content=""
        return content
    ...
    def main_function(self):
        ...
        html_home=fn_get_num_page.get_html("https://www.mlc.gov.tw/latestevent/index.aspx?Parser=9,3,25,,,,,,,,0")
        num_page=fn_get_num_page.get_num_page(html_home,'//a[@title="最末頁"]/@href')
        fn_merge_page_content=GetAllContent()
        for i in range(num_page):
            html_page=fn_merge_page_content.get_html(f"https://www.mlc.gov.tw/latestevent/index.aspx?Parser=9,3,25,,,,,,,,{i}")
            result=fn_merge_page_content.merge_page_content(html_page)
            if(result==False):
                break
        list_total=fn_merge_page_content.get_list_total()
        fn_merge_page_content.write_to_json("miaoliNews",list_total)
